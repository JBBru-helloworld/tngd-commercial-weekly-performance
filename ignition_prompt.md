...
Chunk 5 — Per-L3 merchant analysis: process each of the 7 L3 categories one at a time. For each L3 compute: Top Merchants, Winners, Losers, Momentum, Reactivated, New Entrants, DDNQR. Confirm completion of each L3 before moving to the next.

Chunk 6 — Global DDNQR and Risky Business: apply MID = EP142731 filter across all commercial data for the global DDNQR Top 10. Compute Concentration Risk across all L2 pillars. Assess Early Warning Signals. DDNQR Penetration Tracker TPV source: the 3 summary footer rows require TPV data not revenue. Identify the TPV field in the source data. Compute: Total DDNQR TPV = sum of TPV for MID = EP142731 merchants (YTD, MTD, LW, TW); Total Commercial TPV = sum of TPV across all commercial merchants (computed separately from revenue totals — different field); DDNQR Migration % = DDNQR TPV / Commercial TPV × 100 per column. If TPV field not found: flag in validation summary, insert 'TPV field not found' in all Penetration Tracker cells, proceed with rest of report.

...
- [ ] **DDNQR revenue scoping validated.** Confirm that all DDNQR revenue figures (YTD, MTD, LW, TW, WoW RM, WoW %) will be sourced exclusively from rows where MID = EP142731. Do not use a merchant's total revenue across all MIDs as a proxy for its DDNQR revenue. The correct processing order is: (1) filter all rows to MID = EP142731, (2) group by merchant_group, (3) sum revenue within each group. Confirm this scoping is applied to both the Global Top 10 and all 7 per-L3 Top 5 DDNQR tables, and to the Total DDNQR Revenue footer rows in each table.
...
- [ ] **Mojibake scan complete — Tier 1 and Tier 2 detection applied.** Scan all text columns — particularly merchant_group, category hierarchy fields, and any label columns — using both detection tiers:
  - **Tier 1 (explicit patterns):** Flag any row containing: Ã followed by any character; â€ followed by any character; Å followed by any character; ä» or æ or ç or è or é followed by unexpected characters; å followed by any character; Â followed by any character; Ð or Ñ followed by unexpected characters. A single match triggers the full decoding pipeline.
  - **Tier 2 (structural patterns):** Flag any row where three or more consecutive accented/non-ASCII characters appear in a Latin-script field; mixed encoding types appear in a single string; or CJK Unicode characters (U+4E00–U+9FFF Chinese, U+3040–U+30FF Japanese, U+AC00–U+D7A3 Korean) appear in a field expected to contain Malay, English, or numeric content.
  - **4-pass decoding pipeline:** Pass 1 — Latin-1 → UTF-8; Pass 2 — Windows-1252 → UTF-8; Pass 3 — double-encoding correction; Pass 4 — partial decode + hex-escape remainder + flag as 'manually review required'. Stop at first accepted result.
  - **Reporting requirements:** Report per-pass correction counts; at least one before/after example per corrective pass; list all Pass 4 rows individually with original garbled value.
  - **Partial decodes (Pass 4):** Must be included in analysis using best-available decoded value — do not discard.
  - **Escalation:** If all four passes fail, preserve original, flag the row, and await user instruction before using in ranked tables.
...
- [ ] **Empty table assessment complete.** For each bucket table and DDNQR table (both global Top 10 and all 7 per-L3 Top 5 tables), confirm whether qualifying rows exist after filtering and noise threshold application. List each table that will be empty in the validation summary using the format: 'Will be removed (empty): [TABLE NAME] — [L3 Category or Global]'. Confirm this assessment before generating the HTML output so removals are planned and do not create structural errors.
